...
import json
import os
from candidate_algo import generate_best_candidates
# from .candidate_algo import generate_best_candidates
import openai
...
class SelfHealingAgent:
    ...
    def process_failed_locators(self):
        ...
        try:
            ...
            for index, failure in enumerate(failures.get("failures", []), start=1):
                ...
                # Generate candidates using the algorithm
                candidates = generate_best_candidates(locator_value)
    ...
    def heal_locator(self, locator):
        ...
        try:
            ...
            # Generate candidates using the algorithm
            candidates = generate_best_candidates(locator)
            
            if not candidates:
                logger.console("❌ No candidates found for healing")
                return None
                
            if candidates[0].get("message"):
                logger.console(f"⚠️ Error generating candidates: {candidates[0]['message']}")
                return None
