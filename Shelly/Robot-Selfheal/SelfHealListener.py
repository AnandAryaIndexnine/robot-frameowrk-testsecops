...
class SelfHealListener:
    ...
    def _extract_locator_from_error(self, error_msg):
        ...
        try:
            # Common error patterns with their regex groups
            patterns = [
                # Pattern for 'Page should have contained element'
                r"Page should have contained element ['\"](.*?)['\"] but did not",
                # Pattern for 'Element did not appear'
                r"Element ['\"](.*?)['\"] did not appear",
                # Pattern for 'Element not found'
                r"Element ['\"](.*?)['\"] not found",
                # Generic element pattern
                r"Element ['\"](.*?)['\"]"
            ]
            ...
            for pattern in patterns:
                ...
                if match:
                    # Get the complete locator including quotes
